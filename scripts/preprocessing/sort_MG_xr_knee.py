...
DATASET_DIR = os.path.join(DATA_DIR, "downloads/MG_xr_knee")
...
def setup_directories():
    ...
    print("    2. In subfolder data/train/xr_knee/xr_knee_MG (recommended)")
    print("    (Note: option 2 requires manual review and transfer before training)")
    choice = input("Enter 1 or 2: ")
    print("")

    if choice != "1":
        target_train_dir = os.path.join(SORTED_TRAIN_DIR, "xr_knee_MG")
        target_val_dir = os.path.join(SORTED_VAL_DIR, "xr_knee_MG")
...
def list_filepaths():
    all_filepaths = []
    for subdir in ["Normal", "Osteopenia", "Osteoporosis"]:
